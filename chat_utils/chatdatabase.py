...
class ChatDatabase:
    ...
    async def get_message_log(self, user):
        query_get_last_online = "Select last_online from user where username = ?"
        await self.cursor.execute(query_get_last_online, (user.username, ))
        ...
        if messages != []:
            for message in messages:
                if message[1] == "broadcast":
                    user.streams.write(f"[{message[2]}]{message[0]}: {message[3]}")
                elif message[1] == user.username:
                    user.streams.write(f"[{message[2]}]{message[0]}(to you): {message[3]}")
        await user.streams.write('1\n')
